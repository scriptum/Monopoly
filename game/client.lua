...
cheetah.newThread('game/server.lua')
...
--позиции угловых клеток
local corners = {1, field_width + 2, field_width + field_height + 3, field_width * 2 + field_height + 4}
...
move = function(num, ds1, ds2)
	...
	if pl.jail == 0 then --не знаю, нужно ли на клиенте проверять, сидит ли он в тюрьме? По теории это нужно в сервер
		...
		for i = 1, x do
			pl.pos = pl.pos + 1
			if pl.pos > cell_count then
				pl.pos = pl.pos - cell_count
			end
			if table.find(corners, pl.pos) > 0 then
...
for k = 1, 5 do
	...
	E:new(players)
	:draw(player_draw)
	:set{k=k, jail=0, pos=1, x=x, y=y, cash = 0}
end
