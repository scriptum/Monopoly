...
function(s)
  ...
  for i = 1+s.start, N+s.start do
    S.print(s.lines[i], s.x, s.y + (i - s.start) * fh - fh)
  end
end)
...
callback = function(s)
  local p = s._parent
  p.start = math.floor((s.y - p.y)/(p.h-50)*(#p.lines-math.floor(p.h/p.font:height())))
end})
